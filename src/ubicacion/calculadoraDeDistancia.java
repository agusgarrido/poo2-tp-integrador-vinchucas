...
public class calculadoraDeDistancia {
	...
	//Ver si podemos migrar el Math.toRadians a la ubicacion para no repetir codigo. 
    public static double distanciaEntreDosUbicaciones(Ubicacion ubicacion1, Ubicacion ubicacion2) {
        ...
        double a = Math.pow(Math.sin(deltaLat / 2), 2)
                 + Math.cos(Math.toRadians(ubicacion1.getLatitud())) * Math.cos(Math.toRadians(ubicacion2.getLatitud()))
    ...
    public List<Ubicacion> ubicacionesCercanasA(Ubicacion referencia, List<Ubicacion> ubicaciones, double distanciaMetros) {
    ...
    public List<Muestra> muestrasCercanasA(Muestra muestraReferencia, List<Muestra> muestras, double distanciaMetros){
    	return muestras.stream().filter(muestra-> this.distanciaEntreDosUbicaciones(muestraReferencia.getUbicacion(),muestra.getUbicacion())<=distanciaMetros).collect(Collectors.toList());
    }
