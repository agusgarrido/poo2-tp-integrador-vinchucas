...
public class SistemaWeb {
	...
	private void notificarOrganizacionesNuevaMuestra(Muestra muestra) {
		this.zonas.stream().filter(z -> z.contiene(muestra)).forEach(z -> z.notificarNuevaMuestra(muestra));
	}

	private void notificarOrganizacionesMuestraValidada(Muestra muestra) {
		this.zonas.stream().filter(z -> z.contiene(muestra)).forEach(z -> z.notificarMuestraValidada(muestra));
	}
